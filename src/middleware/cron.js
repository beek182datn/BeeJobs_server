...
const deleteUnverifiedUsers = async () => {
  try {
    ...
    const unverifiedUsers = await User.userModel.find({
      active: StatusUser.INACTIVE,
      create_at: { $lt: oneMinuteAgo },
    });

    const unverifiedJobs = await jobModel.find({
    ...
    if (unverifiedJobs.length > 0) {
      ...
      await jobModel.findByIdAndUpdate({
        _id: {
          $in: unverifiedJobs.map((job) => job._id),
